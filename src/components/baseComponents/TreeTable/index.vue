      ...
      <el-table-column v-if="operator&&treeType === 'isEdit'" label="操作" min-width="120">
        <template slot-scope="scope">
          <el-button
            ...
            :type="item.type"
            size="small"
            v-show="(!item.show || item.show.val.includes(scope.row[item.show.key]))&&(item.resCode?btnPremissions.includes(item.resCode):true)"
          >{{item.text}}</el-button>
        </template>
      </el-table-column>
      <slot></slot>
  ...
  methods: {
    ...
    toggleChecked (row) {
      ...
      if (row.children.length > 0) {
        this.selectionChildren(row, row.check)
        if (row._level !== 0) {
          this.selectionParent(this.dataSource, row.id, row.check, row)
        }
      } else {
        this.selectionParent(this.dataSource, row.id, row.check, row)
      }
    ...
    // 子找父递归
    selectionParent (data, id, check, row) {
      let result = []
      ...
      if (check) {
        ...
        result && result.map(item => {
          ...
          if (!isSelect && isSelect1 && !isSelect2) item.check = false
        })
      }
    },
    // 父节点子节点联动（父找子）
