  ...
  <div v-if="!item.isShow" class="menu-wrapper">
    ...
    >
      ...
      >
        ...
        >
          <item
            :icon="isIcon&&(onlyOneChild.meta.icon||(item.meta&&item.meta.icon))"
    ...
    <el-submenu v-else ref="subMenu" :index="resolvePath(item.path)" popper-append-to-body>
      <template slot="title">
        <item
          v-if="item.meta"
          :icon="isIcon&&(item.meta && item.meta.icon)"
...
import { isExternal } from '@/common/utils.js'
...
export default {
  ...
  computed: {
    isIcon () {
      return this.$store.getters.terminalType !== 'mgt'
    }
  },
  mounted () {
    // console.log(7878, this.isIcon)
  },
  methods: {
