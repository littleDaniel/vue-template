<template>
  <div id="tags-view-container" class="tags-view-container">
    <div
      class="tags"
      ref="tags"
      :style="{borderLeft:$store.getters.terminalType === 'mgt'?'none':'1px solid #ccc'}"
    >
      ...
      <div class="right-tag flex-box" ref="rightTag">
        ...
        <div
          class="returnPortal"
          @click="returnPortals"
          v-if="$store.getters.terminalType !== 'mgt'"
        >返回门户</div>
...
export default {
  ...
  methods: {
    ...
    // 获取页签距离左边
    fixedLeft () {
      return this.device === 'mobile' ? '10px' : this.sidebar ? `${parseInt(this.variables.sideBarWidth) + 10}px` : `${parseInt(this.variables.hideSidebarWidth) + 10}px`
    },
    // 返回门户
    returnPortals () {
      this.$store.dispatch('user/resetToken').then(() => {
        window.location.href = `${process.env.VUE_APP_JUMP_URL}?redirectTo=null&isFrame=false&entId=${this.$store.getters.redirectEntId}&userId=${this.$store.getters.entUserId}`
      })
    },
    closeTag (key) {
      switch (key) {
    ...
    addTags () {
      ...
      if (path) {
        ...
        // if (this.isTabClick) {
        //   this.$store.dispatch('tagsView/addCachedViewBlank', this.$route)
        // } else {
    ...
    */
    handleClick (tab, event) {
      this.$store.commit('tagsView/IS_TAB_CLICK', true)
      // 找到点击的页面在 tag 列表里是哪个
     ...
     */
    handleTabsEdit (view) {
      // this.$store.commit('tagsView/IS_TAB_CLICK', false)
      const page = this.visitedViews.find(page => {
        if (page.fullPath.includes('?')) {
